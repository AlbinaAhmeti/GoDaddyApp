...
function launchChromeForDebugging() {
  ...
  const chrome = spawn(
    ...
    [
      ...
      "--user-data-dir=C:\\chrome-godaddy"
    ],
    {
      detached: true,
      stdio: "ignore"
...
async function collectDomainsFromTable(page) {
  ...
  await page.waitForSelector('a[href*="/control/portfolio/"]', { timeout: 15000 });

  const domains = await page.evaluate(() => {
    const results = new Set();
    const anchors = Array.from(document.querySelectorAll('a[href*="/control/portfolio/"]'));
...
async function goToDomainForwarding(page, domain) {
  ...
  const clickedDns = await waitAndClick(page, [
    'a:has-text("DNS")',
    'button:has-text("DNS")',
    'text="DNS"'
  ], 8000);
  ...
  const clickedForwarding = await waitAndClick(page, [
...
async function runBot(log) {
  ...
  const domains = await collectDomainsFromTable(page);
